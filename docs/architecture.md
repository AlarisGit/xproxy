...
- **Status journal и notifier:** история состояний в SQLite и сводки Telegram через проверенный xray после восстановления proxy или раз в сутки.
...
5. При ошибке восстановить точную предыдущую версию из журнала, проверить запуск и отдельно — трафик после отката. Ошибка отката отражается в логе; восстановление файла не объявляется восстановлением канала.
...
| `status.sqlite3` | История direct/proxy/VLESS/SSH и даты отправленных сводок |
