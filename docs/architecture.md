...
`Connectivity` хранит ограниченное по времени свидетельство доступа в интернет, поколение и локальную сигнатуру default route. Чтение этого объекта не выполняет I/O. Доступ определяется IP-пробами напрямую; уже существующий рабочий proxy также может подтвердить сеть, если прямые checker-адреса фильтруются. При отрицательном результате перед продвижением по SSH-списку используется консервативная прямая перепроверка. Недоступность всех проверочных ресурсов может задержать аварийное подключение: она не даёт оснований обходить скрытые SSH-хосты.
