...
Для стабильности xray мы **никогда** не заменяем боевой `.dat`, пока не убедились, что новый файл полноценный:

1. Скачивание идёт в уникальный временный файл `<name>.part` в той же директории (atomic `os.replace` работает только в рамках одной FS).
...
3. Только при успешной валидации выполняется `os.replace(tmp, target)` — замена атомарна. При любой ошибке `.part` удаляется, боевой файл остаётся нетронутым.
4. При старте и на каждой итерации файл на диске проверяется тем же парсером: если уже лежит битый `.dat` (например, опубликованный старой версией кода до введения валидации), он принудительно перескачивается, не дожидаясь `GEO_REFRESH`.
...
`routing.json` может содержать ссылки на категории (`geosite:vk`, `geoip:ru`, `geosite:category-ru` и т. п.), которых может не быть в конкретном `.dat`. Если такие ссылки пройдут в `config.json`, `xray -test` упадёт — и демон не сможет применить конфиг даже к валидному серверу. Поэтому `build_xray_sections()`:
...
- **Валидный `.dat`, категория отсутствует** → ссылка вырезается из routing/DNS (нормальная очистка).
