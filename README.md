...
- [Состояния и сводки Telegram](docs/notifications.md)
...
Логи: `/var/log/xproxy/xproxy.log`, 100 КБ × 4 файла, плюс stderr. При отсутствии прав на лог-каталог остаётся stderr. SSH пишет локальный `state/ssh.log` с ротацией между попытками; этот файл может содержать адрес текущего хоста. История состояний хранится в `state/status.sqlite3`; сводка Telegram использует идентификатор туннеля вместо его адреса.
